#A3C model implementation taken from https://github.com/eugene/pommerman
...
GAMMA = 0.95
LAMBDA = 1
MAX_EP = 25000 # number of training games
...
#Save model checkpoint after Max episodes reached
...
#A3C Neural Net Model definition 
...
class Worker(mp.Process):
    ...
    def run(self):
        ...
        self.agentList = [self.A3CAgent, agents.SimpleAgent(), agents.SimpleAgent(), agents.RandomAgent()]
        ...
        while self.g_ep.value < MAX_EP:
            ...
            while True:
                ...
                done = done or r[self.agent_nr]==-1#not(10 in s_new[self.agent_nr]['alive']) #if done or agent 10 is dead
...
def main():
    ...
    opt = SharedAdam(gnet.parameters(), lr=0.00001)      # global optimizer
    ...
    global_ep, global_ep_r,global_nr_steps, res_queue = mp.Value('d', 0), mp.Value('d', 0.), mp.Value('d', 0.), mp.Queue()
     ...
     #parallel training
    workers = [Worker(gnet, opt, global_ep, global_ep_r, global_nr_steps, res_queue, i) for i in range(mp.cpu_count())]
